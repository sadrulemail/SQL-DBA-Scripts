...
from JobMain jsch
left join (
select sJOB.job_id, name, case when name like '%full%' then 'FULL'  when name like '%diff%' then 'DIFF'  when name like '%log%' then 'LOG' 
			when name like '%tran%' then 'LOG'  when name like '%trn%' then 'LOG' when name like '%sysdb%' then 'FULL' else 'FULL' end as bak_typr,
command,
...
and  [sJOB].name not like '%delete%' and [sJOB].name not like '%update%' and [sJOB].name not like '%stats%' and [sJOB].name not like '%Recycle%'
 ...
 where j.Enabled = 'YES'
	...
	and ( j.Step_Command not like '%shrink%' )
...
where  db.database_id <> 2 and db.state_desc = 'online'
 ...
 from AOAGdetails ag
 left join jobdetails jd on  jd.Schedule not like '%No Schedule Found%' and jd.Schedule not like '%one time%' 
 and 
 /* only database name*/   ( (jd.[Databases] like  '%'+ag.[Database]+'%' AND jd.[Databases] not like  '%-'+ag.[Database]+'%' )
 /* user database */      or ((jd.[Databases] like '%USER_DATABASES%' OR jd.[Databases] like '%ALL_DATABASES%') and jd.[Databases] not like '%-AVAILABILITY_GROUP_DATABASES%' AND jd.[Databases] not like  '%-'+ag.[Database]+'%' and ag.database_id > 4) 
 /* AG database Native */ or (jd.[Databases] like '%AVAILABILITY_GROUP_DATABASES%' and jd.[Databases] not like '%USER_DATABASES%' AND jd.[Databases] not like  '%-'+ag.[Database]+'%' and [Cluster Type] = 'Always-On' and ag.database_id > 4) 
 /* AG database SM*/      or (jd.[Databases] like '%-ag%' and [Cluster Type] = 'Always-On' and ag.database_id > 4) 
 /* non-AG db Native */   or (jd.[Databases] like '%-AVAILABILITY_GROUP_DATABASES%' and jd.[Databases] like '%USER_DATABASES%' AND jd.[Databases] not like  '%-'+ag.[Database]+'%' and [Cluster Type] = '' and ag.database_id > 4) 
 /* system database*/     or ((jd.[Databases] like '%SYSTEM_DATABASES%' OR jd.[Databases] like '%ALL_DATABASES%') and ag.database_id <= 4) )
 )
